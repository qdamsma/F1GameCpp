...
void Screen::Draw(const BMPImage& image, const Sprite& sprite, const Vec2D& pos, const Color& overlayColor, float rotation)
{
    ...
    // Bereken midden van sprite
    Vec2D center = pos + Vec2D(width/2.0f, height/2.0f);

    // Draai de hoeken rond het midden
    topLeft.Rotate(rotation * (3.14159265f / 180.0f), center);
    topRight.Rotate(rotation * (3.14159265f / 180.0f), center);
    bottomLeft.Rotate(rotation * (3.14159265f / 180.0f), center);
    bottomRight.Rotate(rotation * (3.14159265f / 180.0f), center);
